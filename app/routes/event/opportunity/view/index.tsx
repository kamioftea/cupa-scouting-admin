...
export async function loader({context}: Route.LoaderArgs) {
  ...
  const followedByEncounterIds = await scoutingRepository.getFollowUpEncounterIds(opportunity.opportunityId);
  const sourceEncounterIds = await scoutingRepository.getSourceEncounterIds(opportunity.opportunityId);
  ...
  return {
    links: {
      ...
      sourceEncounter: {
        ...
        header: "Source encounters",
        options: (await metadataRepository.findEncountersByEvent(event.eventId))
          .filter(
            ({encounterId}) =>
              !followedByEncounterIds.includes(encounterId)
          )
          .map(
            ({encounterId, code, name}) =>
              ({
                value: encounterId,
                label: `${code} - ${name}`,
                url: `/event/${event.slug}/encounter/${encounterId}`
              })
          ),
        selected: sourceEncounterIds
      },
      followUpEncounter: {
        label: "Follow up encounter",
        header: "Follow up encounters",
        options: (await metadataRepository.findEncountersByEvent(event.eventId))
          .filter(
            ({encounterId}) =>
              !sourceEncounterIds.includes(encounterId)
          )
          .map(
            ({encounterId, code, name}) =>
              ({
        ...
        selected: followedByEncounterIds
      }
    }
  };
}
...
async function addLink(
  ...
  switch (linkType) {
    ...
    case "sourceEncounter":
      await scoutingRepository.linkSourceEncounter(opportunityId, linkId);
      return redirect('./');
    case "followUpEncounter":
      await scoutingRepository.linkFollowUpEncounter(opportunityId, linkId);
      return redirect('./');
    default:
...
async function removeLink(
  ...
  switch (linkType) {
    case "statBlocks":
      await scoutingRepository.unlinkStatBlockFromOpportunity(opportunityId, linkId);
      return redirect('./');
    case "npcs":
      await scoutingRepository.unlinkNPCFromOpportunity(opportunityId, linkId);
      return redirect('./');
    case "snippets":
      await scoutingRepository.unlinkSnippetFromOpportunity(opportunityId, linkId);
      return redirect('./');
    case "followedBy":
      await scoutingRepository.unlinkOpportunities(opportunityId, linkId);
      return redirect('./');
    case "follows":
      await scoutingRepository.unlinkOpportunities(linkId, opportunityId);
      return redirect('./');
    case "sourceEncounter":
      await scoutingRepository.unlinkSourceEncounter(opportunityId, linkId);
      return redirect('./');
    case "followUpEncounter":
      await scoutingRepository.unlinkFollowUpEncounter(opportunityId, linkId);
      return redirect('./');
    default:
      return {errors: [{message: "Invalid link type", field: "linkType"}]};
  }
...
export async function action({request, context}: Route.ActionArgs) {
  ...
  switch (action) {
    case "link":
      return addLink(linkType, scoutingRepository, opportunityId, linkId);
    
    case "unlink":
      return removeLink(linkType, scoutingRepository, opportunityId, linkId);
  }
    ...
    {currentSpec == null
     ? <div className="button-group small">
       {Object.entries(specs).map(
         ([key, {label}]) =>
           <button key={key} className="button info" onClick={() => setAddLinkType(key)}>
             Add {label}
           </button>
       )}
     </div>
     : <form method="post" action="?index">
       ...
       />
       <div className="button-group small">
         <button type={'submit'} className="button info" name={"action"} value={"link"}>
        ...
        <div key={key} className="link-group">
          ...
          <ul>
            {selected.map((id) => {
              ...
              const content = url ? <Link to={url}>{label}</Link> : label;
              
              return <li key={id}>
                {content}
                <form method="post" action="?index" className="display-inline">
                  <input type="hidden" name="linkType" value={key}/>
                  <input type="hidden" name="linkId" value={id}/>
                  <button type="submit" name="action" value="unlink" className="button text-alert link">
                    [Remove]
                  </button>
                </form>
              </li>;
