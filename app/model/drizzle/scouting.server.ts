...
import {
  ...
  opportunityNPCs,
  type OpportunityRow, opportunitySourceEncounters,
...
export class DrizzleScoutingRepository {
  ...
  async unlinkSnippetFromOpportunity(opportunityId: number, snippetId: number): Promise<void> {
    await this.db
              .delete(opportunityInformationSnippets)
              .where(and(
                eq(opportunityInformationSnippets.opportunityId, opportunityId),
                eq(opportunityInformationSnippets.snippetId, snippetId)
              ));
  }
  ...
  async unlinkStatBlockFromOpportunity(opportunityId: number, statBlockId: number): Promise<void> {
    await this.db
              .delete(opportunityStatBlocks)
              .where(and(
                eq(opportunityStatBlocks.opportunityId, opportunityId),
                eq(opportunityStatBlocks.statBlockId, statBlockId)
              ));
  }
  ...
  async unlinkNPCFromOpportunity(opportunityId: number, npcId: number): Promise<void> {
    await this.db
              .delete(opportunityNPCs)
              .where(and(
                eq(opportunityNPCs.opportunityId, opportunityId),
                eq(opportunityNPCs.npcId, npcId)
              ));
  }
  ...
  async unlinkOpportunities(sourceOpportunityId: number, unlockedOpportunityId: number) {
    await this.db
              .delete(opportunityFollowUps)
              .where(and(
                eq(opportunityFollowUps.sourceOpportunityId, sourceOpportunityId),
                eq(opportunityFollowUps.unlockedOpportunityId, unlockedOpportunityId)
              ));
  }
  
  async getSourceEncounterIds(opportunityId: number) {
    const sourceEncounters = await
      this.db
          .select({encounterId: opportunitySourceEncounters.encounterId})
          .from(opportunitySourceEncounters)
          .where(eq(opportunitySourceEncounters.opportunityId, opportunityId));
    
    return sourceEncounters.map(s => s.encounterId);
  }
  
  async linkSourceEncounter(opportunityId: number, encounterId: number) {
    await this.db
              .insert(opportunitySourceEncounters)
              .values({opportunityId, encounterId})
              .onConflictDoNothing();
  }
  
  async unlinkSourceEncounter(opportunityId: number, encounterId: number) {
    console.log(`unlinkSourceEncounter: opportunityId=${opportunityId}, encounterId=${encounterId}`);
    await this.db
              .delete(opportunitySourceEncounters)
              .where(and(
                eq(opportunitySourceEncounters.opportunityId, opportunityId),
                eq(opportunitySourceEncounters.encounterId, encounterId)
              ));
  }
  
  async getFollowUpEncounterIds(opportunityId: number) {
    const followUpEncounters = await
      this.db
          .select({encounterId: opportunityFollowUpEncounters.encounterId})
          .from(opportunityFollowUpEncounters)
          .where(eq(opportunityFollowUpEncounters.opportunityId, opportunityId));
    
    return followUpEncounters.map(s => s.encounterId);
  }
  
  async linkFollowUpEncounter(opportunityId: number, encounterId: number) {
    await this.db
              .insert(opportunityFollowUpEncounters)
              .values({opportunityId, encounterId})
              .onConflictDoNothing();
  }
  
  async unlinkFollowUpEncounter(opportunityId: number, encounterId: number) {
    console.log(`unlinkFollowUpEncounter: opportunityId=${opportunityId}, encounterId=${encounterId}`);
    await this.db
              .delete(opportunityFollowUpEncounters)
              .where(and(
                eq(opportunityFollowUpEncounters.opportunityId, opportunityId),
                eq(opportunityFollowUpEncounters.encounterId, encounterId)
              ));
  }
...
export const opportunityValidator: z.Schema<Omit<OpportunityRow, 'opportunityId' | 'eventId' | 'code'>> = z.object(
  {
    ...
    monsterBriefing: z.string().nullable().default(null),
    expectedResult: z.string().nullable().default(null),
  }
);
