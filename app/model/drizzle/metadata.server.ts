...
import {
  ...
  type StatBlockRow,
  statBlocks
} from "~/model/drizzle/schema/metadata";
...
export class DrizzleMetadataRepository {
  ...
  findEncountersByEvent(eventId: number): Promise<EncounterRow[]> {
    return this.db.select().from(encounters).where(eq(encounters.eventId, eventId));
  }
  
  findEncounterOpportunitiesByEvent(eventId: number): Promise<EncounterRow[]> {
    return this.db
               .select()
               .from(encounters)
               .where(and(
                 eq(encounters.eventId, eventId),
                 eq(encounters.produceEncounterOpportunity, true)
               ));
  }
  
  findEncounter(encounterId: number): Promise<EncounterRow | undefined> {
    return this.db.query.encounters.findFirst({where: eq(encounters.encounterId, encounterId)});
  }
  
  async insertEncounter(encounter: Omit<EncounterRow, 'encounterId'>): Promise<number> {
    const res =
      await this.db
                .insert(encounters)
                .values(encounter)
  ...
  async updateEncounter(encounterId: number, encounter: Omit<EncounterRow, 'encounterId'>): Promise<void> {
    await this.db
              .update(encounters)
              .set(encounter)
